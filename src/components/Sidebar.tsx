...
export function Sidebar({ activeView, onViewChange }: SidebarProps) {
  const {
    conversations, currentConversationId, sidebarOpen, analyticsPanelOpen,
    createConversation, deleteConversation, setCurrentConversation,
    toggleSidebar, toggleAnalyticsPanel, pinConversation, updateConversationTitle,
  } = useChatStore()
  ...
  const startEdit = (conv: Conversation) => { setEditingId(conv.id); setEditTitle(conv.title) }
  ...
  const borderColor = isDark ? '#7c3aed' : '#1a1a2e'
  const shadowColor = isDark ? '#7c3aed' : '#1a1a2e'
  ...
  const ConversationItem = ({ conv }: { conv: Conversation }) => {
    ...
    return (
      ...
      >
        <MessageSquare className={cn('h-4 w-4 shrink-0', isActive ? 'text-purple-400' : 'text-gray-400')} />
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-44">
            ...
            <DropdownMenuItem className="text-red-500 focus:text-red-500 focus:bg-red-50" onClick={() => deleteConversation(conv.id)}>
    ...
    <AnimatePresence initial={false}>
      {sidebarOpen && (
        <motion.aside
          ...
          className={cn(
            'flex flex-col h-full overflow-hidden shrink-0 border-r-[2.5px] transition-colors duration-200',
            isDark ? 'bg-[#13132a] border-purple-600' : 'bg-white border-gray-900'
          )}
          style={{ boxShadow: `4px 0 0 ${shadowColor}` }}
        >
          {/* Header */}
          <div className={cn('flex items-center justify-between px-4 py-4 border-b-[2.5px]', isDark ? 'border-purple-600' : 'border-gray-900')}>
            <div className="flex items-center gap-2">
              <div
                className="h-8 w-8 rounded-xl bg-gradient-to-br from-purple-600 to-purple-400 flex items-center justify-center border-2"
                style={{ borderColor, boxShadow: `2px 2px 0 ${shadowColor}` }}
              >
                <MessageSquare className="h-4 w-4 text-white" />
              </div>
              <span className={cn('font-bangers text-lg tracking-wide', isDark ? 'text-purple-300' : 'text-gray-900')}>
                Made ChatBot
              </span>
            </div>
            <div className="flex items-center gap-1">
              ...
              <button
                onClick={toggleTheme}
                className={cn(
                  'rounded-lg p-1.5 transition-all duration-200 border',
                  isDark
                    ? 'text-yellow-400 hover:bg-purple-900/40 border-purple-700'
              ...
              <button
                ...
                className={cn(
                  ...
                  activeView === id
                    ...
                    : isDark
                      ? 'bg-white/5 text-gray-400 border-purple-800 hover:bg-purple-900/30 hover:text-purple-300'
            ...
            <div className="px-3 pt-2">
              <div className="relative">
                ...
                <Input
                  ...
                  className={cn('pl-8 h-8 text-xs border-2', isDark ? 'bg-white/5 border-purple-800 text-gray-200 placeholder:text-gray-500' : 'border-gray-200')}
                />
              </div>
            </div>
          )}
          ...
          {activeView === 'chat' && (
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1">
              {pinned.length > 0 && (
                <>
                  <p className="px-2 py-1 text-xs font-bangers text-purple-400 uppercase tracking-wider">📌 Pinned</p>
                  <AnimatePresence>{pinned.map((conv) => <ConversationItem key={conv.id} conv={conv} />)}</AnimatePresence>
                  {unpinned.length > 0 && <p className="px-2 py-1 mt-2 text-xs font-bangers text-gray-500 uppercase tracking-wider">Recent</p>}
                </>
              )}
          ...
          {/* Footer */}
          <div className={cn('px-3 py-3 border-t-[2.5px] space-y-1', isDark ? 'border-purple-700' : 'border-gray-900')}>
            <button
            ...
            {/* Creator card */}
            <div className={cn('rounded-2xl border-2 border-purple-500 px-3 py-2.5 mt-1', isDark ? 'bg-purple-900/20' : 'bg-purple-50')}
              style={{ boxShadow: '3px 3px 0 #7c3aed' }}>
              <p className="font-bangers text-base text-purple-500 tracking-wide">Madeha Shah</p>
              <a
                href="mailto:ms8645960@gmail.com"
                className={cn('flex items-center gap-1.5 text-xs hover:text-purple-500 transition-colors mt-0.5', isDark ? 'text-gray-400' : 'text-gray-500')}
              >
